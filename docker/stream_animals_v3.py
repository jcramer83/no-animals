...
FFPROBE = shutil.which("ffprobe") or os.environ.get("FFPROBE_PATH") or str(Path(FFMPEG).parent / "ffprobe")
...
class StreamInstance:
    ...
    def run_pipeline(self):
        ...
        enc_cmd += [
            ...
            "-preset", "fast", "-b:v", "4M", "-maxrate", "6M", "-bufsize", "8M",
            "-g", str(int(FPS * 2)),
        ]
        if has_audio:
...
def _load_bold_font(size):
    """Load a bold font with Linux/Docker fallbacks."""
    ...
    for path in ("/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf",
...
def _load_regular_font(size):
    """Load a regular font with Linux/Docker fallbacks."""
...
def _load_emoji_font(size):
    """Load an emoji font with Linux/Docker fallbacks. Returns None if unavailable."""
...
def _build_poweredby_overlay():
    ...
    font_sm = _load_regular_font(13)
    font_brand = _load_bold_font(16)
    tmp = Image.new("RGBA", (1, 1))
    d = ImageDraw.Draw(tmp)
...
<option value="yolov8x_openvino_model" selected>yolov8x (accurate)</option>
</select></div>
<div class="setting-desc">YOLOv8 model size (OpenVINO). Larger = more accurate but uses more CPU.</div>

...
- **Do NOT use `-live_start_index` on provider inputs.** It exists in FFmpeg's *HLS demuxer*, but provider URLs are continuous **MPEG-TS** pushes (`format_name=mpegts`), not HLS playlists, so FFmpeg rejects it with `Option not found` and the decoder never starts — surfacing as `Decoder video TCP accept timed out`. An earlier edit to this file "corrected" the original warning after confirming the option exists in isolation; that correction was wrong, because existence in the HLS demuxer says nothing about an MPEG-TS input. Verify against an actual provider URL, not `ffmpeg -h demuxer=hls`.
- **There is no upstream backlog to rewind into.** Because the source is a live TS push, viewer buffer cannot be bought on the input side. It is created on output via `#EXT-X-START:TIME-OFFSET` (see `_inject_live_start()`), which moves where players begin within our own playlist.
...
- **Judge capacity by content-vs-wall-clock, never by fps**: To answer "can the pipeline keep up?", measure how much content the encoder emits per second of wall clock (segment end-PTS advance ÷ elapsed). >=1.0x keeps up; below that the 32s HLS window drains and players freeze. Averaging `stats["fps"]` gives wrong answers — it read 28.1fps (apparent failure) for a config simultaneously measuring 1.197x realtime (comfortable pass). Always discard the first ~2 min, which runs >1x while draining the provider's DVR backlog and hides a real deficit.
- **Detection cost is bounded by `detect_every`, and the rungs are far apart**: On an i9-13900K with yolov8m at 640 — `detect_every=2` → 0.69x realtime (freezes players), `detect_every=4` → >=1.2x (comfortable). 3 interpolates to ~0.9x, i.e. still failing, so there is no safe middle rung. Changing model or `DETECT_IMGSZ` invalidates these numbers; re-measure.
- **`persist_frames` must exceed `detect_every`**: boxes are held for `persist_frames` frames after a detection is lost, but detection only runs every `detect_every` frames. If persist is the smaller number, censor boxes flicker on and off between detections.
